...
# 定义保存分布图的根目录
DISTRIBUTION_SAVE_DIR = "/home/wangyufei/code/SNNTransformer/Spikingformer/cifar10/distribution"
_saved_layer_names = set()  # 用于记录已保存过图像的层名称
...
# def save_distribution_plot(tensor_data, layer_name, step_count):
#     """
#     保存张量数据的分布直方图
#     """
#     try:
...
#         # 将数据移动到CPU并转换为numpy，同时detach梯度
#         data_np = tensor_data.detach().cpu().flatten().numpy()
        
#         # 如果数据量太大，采样以提高绘图速度
#         if data_np.size > 100000:
#             indices = np.random.choice(data_np.size, 100000, replace=False)
#             data_np = data_np[indices]

#         plt.figure(figsize=(10, 6))
#         plt.hist(data_np, bins=50, color='blue', alpha=0.7, edgecolor='black')
#         plt.title(f'Distribution of {layer_name} (Step {step_count})')
#         plt.xlabel('Value')
#         plt.ylabel('Frequency')
#         plt.grid(True, linestyle='--', alpha=0.5)
        
#         # 生成文件名，包含步骤计数以避免覆盖
#         filename = f"{layer_name}_step_{step_count}.png"
#         filepath = os.path.join(DISTRIBUTION_SAVE_DIR, filename)
        
#         plt.savefig(filepath)
#         plt.close()
        
#     except Exception as e:
#         print(f"Error saving distribution plot for {layer_name}: {e}")

def save_distribution_plot(tensor_data, layer_name, step_count, vline_value=None):
    """
    保存张量数据的分布直方图
    """
    # try:
    ...
    # 将数据移动到CPU并转换为numpy，同时detach梯度
    data_np = tensor_data.detach().cpu().flatten().numpy()
    vline_value = vline_value.detach().cpu().numpy()
    ...
    # --- 新增代码开始 ---
    # 如果提供了 vline_value，则在图中绘制一条竖线
    if vline_value is not None:
        plt.axvline(x=vline_value, color='red', linestyle='--', linewidth=2, label=f'Value: {vline_value}')
        plt.legend() # 显示图例
    # --- 新增代码结束 ---
...
class mem_update(nn.Module):
    def __init__(
        ...
        **neuron_kwargs,
        ...
        # 给每个实例一个唯一ID，方便在文件名中区分不同的层
        global _global_layer_counter 
        self.layer_id = _global_layer_counter
        _global_layer_counter += 1

    def forward(self, x, bn=None):
        global _global_step_counter
        
        # --- 新增：统计x值分布 ---
        # 仅在训练模式下或者特定条件下记录，避免评估时产生大量文件
        # 这里为了演示，每次forward都记录，但实际使用中建议加限制条件
        # 例如：if self.training and _global_step_counter % 100 == 0:
        
        # 为了性能，我们每隔一定步数保存一次，或者只在第一步保存
        # 这里简单实现：每调用100次保存一次，防止IO瓶颈
        # if _global_step_counter % 100 == 0:
        #     # 获取当前模块的类名和ID作为标识
        #     layer_name = f"mem_update_{self.layer_id}"
        #     save_distribution_plot(x, layer_name, _global_step_counter)
        ...
        layer_name = f"mem_update_{self.layer_id}"
        global _saved_layer_names
        if layer_name not in _saved_layer_names:
            # 3. 如果没有保存过，则执行保存操作
            # 这里我们固定使用 step 0，因为每个层只保存一次
            save_distribution_plot(x, layer_name, step_count=0, vline_value=self.qtrick.upper_bound_mean[-2])
            
            # 4. 将该层名称添加到已保存集合中，防止下次重复保存
            _saved_layer_names.add(layer_name)
