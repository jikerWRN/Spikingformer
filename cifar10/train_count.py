...
parser.add_argument('--spike-percentile', type=float, nargs='+', default=[0.7, 0.8, 0.9, 0.99],
                    help='positive activation percentiles tracked by adaptive spike modules')
...
def load_state_dict_flexible(model, state_dict, label):
    model_state = model.state_dict()

    if all(k.startswith('module.') for k in model_state.keys()) and not any(k.startswith('module.') for k in state_dict.keys()):
        state_dict = {'module.' + k: v for k, v in state_dict.items()}
    elif not any(k.startswith('module.') for k in model_state.keys()) and all(k.startswith('module.') for k in state_dict.keys()):
    ...
    if skipped_keys:
        _logger.info('Skipped mismatched keys when loading %s: %s', label, skipped_keys)
    if incompatible.missing_keys:
        _logger.info('Missing keys when loading %s: %s', label, incompatible.missing_keys)
    if incompatible.unexpected_keys:
        _logger.info('Unexpected keys when loading %s: %s', label, incompatible.unexpected_keys)


def load_initial_checkpoint(model, checkpoint_path):
    _logger.info('Loading initial checkpoint: %s', checkpoint_path)
    ...
    if isinstance(checkpoint, dict):
        ...
        state_dict = checkpoint
    load_state_dict_flexible(model, state_dict, 'initial checkpoint')


def collect_upper_bound_stats(model):
    ...
    for module_name, module in model.named_modules():
        ...
        count = module.upper_bound_count.item()
        if count <= 0:
            continue

        last_idx = -1
        percentile_value = module.percentile.reshape(-1)[last_idx].item() if hasattr(module, 'percentile') else None
        ...
        stats[prefix + '/raw_last'] = module.upper_bound_raw_last.reshape(-1)[last_idx].item()
        stats[prefix + '/raw_min'] = module.upper_bound_raw_min.reshape(-1)[last_idx].item()
        stats[prefix + '/raw_max'] = module.upper_bound_raw_max.reshape(-1)[last_idx].item()
        stats[prefix + '/raw_avg'] = (module.upper_bound_raw_sum.reshape(-1)[last_idx] / module.upper_bound_count.clamp_min(1.0)).item()
        stats[prefix + '/ema'] = module.upper_bound_mean.reshape(-1)[last_idx].item()
        stats[prefix + '/count'] = count
        layer_idx += 1
    return stats
...
def main():
    ...
    def run_eval(epoch, update=None, train_metrics=None, log_suffix=''):
        ...
        if output_dir is not None:
            train_log_metrics = OrderedDict(train_metrics or OrderedDict([('loss', None)]))
            train_log_metrics.update(collect_upper_bound_stats(model))
            update_summary(
        ...
        if args.log_wandb and has_wandb and args.rank == 0:
            ...
            train_log_metrics = OrderedDict(train_metrics or OrderedDict())
            train_log_metrics.update(collect_upper_bound_stats(model))
            for k, v in train_log_metrics.items():
                if v is not None:
