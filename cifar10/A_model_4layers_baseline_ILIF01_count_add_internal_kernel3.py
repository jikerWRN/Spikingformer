...
class MultiSpike4(nn.Module):
    ...
    def update_upper_bound_mean(self, upper_bound):
        upper_bound = upper_bound.detach().to(self.upper_bound_mean.device, dtype=self.upper_bound_mean.dtype).clamp_min(1e-6)
    ...
    def forward(self, x, bn=None):
        ...
        for i in range(time_window):
            if i >= 1:
                ...
                mem = x[i]
            spike = self.qtrick(mem, shared_upper_bound)
...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0., spike_percentile=(0.7, 0.8, 0.9, 0.99)):
        ...
        self.mlp2_lif = mem_update(spike_percentile=spike_percentile)
...
class SpikingSelfAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1, spike_percentile=(0.7, 0.8, 0.9, 0.99)):
        super().__init__()
        ...
        self.proj_lif = mem_update(spike_percentile=spike_percentile)

        self.q_conv = nn.Conv1d(dim, dim, kernel_size=3, stride=1, padding=1, bias=False)
        self.q_bn = nn.BatchNorm1d(dim)
        self.q_lif = mem_update(spike_percentile=spike_percentile)

        self.k_conv = nn.Conv1d(dim, dim, kernel_size=3, stride=1, padding=1, bias=False)
        self.k_bn = nn.BatchNorm1d(dim)
        self.k_lif = mem_update(spike_percentile=spike_percentile)
        
        self.v_conv = nn.Conv1d(dim, dim, kernel_size=3, stride=1, padding=1, bias=False)
        self.v_bn = nn.BatchNorm1d(dim)
        self.v_lif = mem_update(spike_percentile=spike_percentile)

        self.attn_lif = mem_update(spike_percentile=spike_percentile)
...
class SpikingTransformer1(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1, spike_percentile=(0.7, 0.8, 0.9, 0.99)):
        super().__init__()
        ...
        self.mlp = MLP(in_features=dim, hidden_features=mlp_hidden_dim, drop=drop, spike_percentile=spike_percentile)
...
class SpikingTransformer2(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1, spike_percentile=(0.7, 0.8, 0.9, 0.99)):
        super().__init__()
        self.norm1 = norm_layer(dim)
...
class SpikingTransformer3(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1, spike_percentile=(0.7, 0.8, 0.9, 0.99)):
        super().__init__()
        self.norm1 = norm_layer(dim)
...
class SpikingTransformer4(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1, spike_percentile=(0.7, 0.8, 0.9, 0.99)):
        super().__init__()
        self.norm1 = norm_layer(dim)
...
class SpikingTokenizer(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256, spike_percentile=(0.7, 0.8, 0.9, 0.99)):
        super().__init__()
        ...
        self.block1_lif = mem_update(spike_percentile=spike_percentile)
        ...
        self.block2_lif = mem_update(spike_percentile=spike_percentile)
        ...
        self.block3_lif = mem_update(spike_percentile=spike_percentile)
        ...
        self.block4_lif = mem_update(spike_percentile=spike_percentile)
...
def Spikingformer(pretrained=False, **kwargs):
    allowed_kwargs = {
        ...
        'depths', 'sr_ratios', 'T', 'pretrained_cfg', 'spike_percentile',
    }
