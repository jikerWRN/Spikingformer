...
class MultiSpike4(nn.Module):
    ...
    class quant4(torch.autograd.Function):
        ...
        def forward(ctx, input, training):
            ...
            clamped = torch.clamp(input, min=0, max=3)
            ...
            if training:
                ...
                mask_rand_01 = (clamped > 0.25) & (clamped < 0.75)

                mask_1 = (clamped >= 0.75) & (clamped < 1.25)

                mask_rand_12 = (clamped >= 1.25) & (clamped < 1.75)

                mask_2 = (clamped >= 1.75) & (clamped < 2.25)

                mask_rand_23 = (clamped >= 2.25) & (clamped < 2.75)
                ...
                # (0.25, 0.75) -> 随机 0 / 1
                result[mask_rand_01] = 0.5

                # [0.75, 1.25) -> 1
                result[mask_1] = 1

                # [1.25, 1.75) -> 随机 1 / 2
                result[mask_rand_12] = 1.5

                # [1.75, 2.25) -> 2
                result[mask_2] = 2

                # [2.25, 2.75) -> 随机 2 / 3
                result[mask_rand_23] = 2.5
