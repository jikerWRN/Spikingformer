...
class AdaptiveMultiStepLIFNode(MultiStepLIFNode):
    ...
    def _load_from_state_dict(
        ...
        if checkpoint_mean_key not in state_dict:
            print("checkpoint_mean_key not in state_dict")
        else:
...
class MS_Conv(nn.Module):
    def __init__(self, in_features, spike_selected_percentile=0.99, hidden_features=None, out_features=None, drop=0., spike_percentile=(0.7, 0.8, 0.9, 0.99)):
        super().__init__()
...
class SpikingTransformer(nn.Module):
    def __init__(
        ...
        self.conv1 = MS_Conv(dim, spike_selected_percentile)
        self.conv2 = MS_Conv(dim, spike_selected_percentile)
        self.mlp = MLP(
            in_features=dim,
...
class vit_snn(nn.Module):
    def __init__(
        ...
        print("-----------------{}---------------".format(spike_selected_percentile))
