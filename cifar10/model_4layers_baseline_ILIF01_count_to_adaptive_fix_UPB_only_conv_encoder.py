...
class MultiSpike4_(nn.Module):

    class quant4_(torch.autograd.Function):
    ...
    def forward(self, x):
        return self.quant4_.apply(x)

        
class mem_update_(nn.Module):
    def __init__(self, act=False):
        super(mem_update_, self).__init__()
        # self.actFun= torch.nn.LeakyReLU(0.2, inplace=False)

        self.act = act
        self.qtrick = MultiSpike4_()  # change the max value
...
class SpikingSelfAttention(nn.Module):
    def __init__(
        ...
        self.proj_lif = mem_update_(
        )

        self.q_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.q_bn = nn.BatchNorm1d(dim)
        self.q_lif = mem_update_(
        )

        self.k_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.k_bn = nn.BatchNorm1d(dim)
        self.k_lif = mem_update_(
        )

        self.v_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.v_bn = nn.BatchNorm1d(dim)
        self.v_lif = mem_update_(
        )

        self.attn_lif = mem_update_(
        )
