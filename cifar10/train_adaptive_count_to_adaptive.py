...
parser.add_argument('--spike-percentile', type=float, nargs='+', default=[0.7, 0.8, 0.9, 0.99],
                    help='positive activation percentiles used by adaptive spike upper bound')
parser.add_argument('--spike-selected-percentile', type=float, default=0.99,
...
def load_state_dict_flexible(model, state_dict, label):
    ...
    if skipped_keys:
        _logger.info('Skipped mismatched keys when loading {}: {}'.format(label, skipped_keys))
    if incompatible.missing_keys:
        _logger.info('Missing keys when loading {}: {}'.format(label, incompatible.missing_keys))
    if incompatible.unexpected_keys:
        _logger.info('Unexpected keys when loading {}: {}'.format(label, incompatible.unexpected_keys))


def load_initial_checkpoint(model, checkpoint_path, device):
    _logger.info('Loading initial checkpoint: {}'.format(checkpoint_path))
    ...
    _logger.info('Initial checkpoint loaded. Training will start from epoch 0 unless --start-epoch is set.')
...
def collect_upper_bound_stats(model):
    ...
    for module_name, module in model.named_modules():
        ...
        stats[prefix + '/raw_avg'] = (
            module.upper_bound_raw_sum.reshape(-1)[stat_idx] / module.upper_bound_count.clamp_min(1.0)
        ).item()
...
def main():
    ...
    model = create_model(
        ...
        spike_selected_percentile=args.spike_selected_percentile,
    )
    ...
    if args.initial_checkpoint:
        load_initial_checkpoint(model, args.initial_checkpoint, args.device)
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics.update(collect_upper_bound_stats(model))
