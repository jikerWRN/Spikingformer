...
TENSOR_DISTRIBUTION_DIR = os.path.join(
    ...
    "D_model_4layers_baseline_ILIF01_count_to_adaptive_back_count_kernel3_tensor_distributon",
)
...
class AdaptiveMultiStepLIFNode(MultiStepLIFNode):
        ...
        with torch.no_grad():
            ...
            if values.numel() == 0:
                ...
                hist = torch.histc(
                    ...
                    max=float(values.max().item()) if values.max().item() > values.min().item() else float(values.min().item() + 1.0),
                )
                bin_edges = torch.linspace(
                    float(values.min().item()),
                    float(values.max().item()) if values.max().item() > values.min().item() else float(values.min().item() + 1.0),
    ...
    def multi_step_forward(self, x_seq, bn=None):
        ...
        for t in range(x_seq.shape[0]):
            ...
            assert state.shape == x_seq[t].shape, "State shape does not match input shape at time step {}".format(t)
