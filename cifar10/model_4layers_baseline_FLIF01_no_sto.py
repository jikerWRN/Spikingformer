...
def ratio_0_to_1(tensor):
    """
    计算值在 (0, 1] 区间内的元素，占所有大于0元素的比例
    """
    # 1. 筛选大于 0 的掩码（分母）
    mask_positive = tensor > 0
    total_positive = mask_positive.sum().item()
    
    # 2. 如果没有正数，直接返回 0 或提醒
    if total_positive == 0:
        print("警告：张量中没有大于 0 的值")
        return 0.0
    
    # 3. 筛选大于 0 且 小于等于 1 的掩码（分子）
    # 注意：这里用 <= 1 包含边界 1；如果不想包含 1，改成 < 1 即可
    mask_in_range = (tensor > 0) & (tensor <= 1)
    count_in_range = mask_in_range.sum().item()
    
    # 4. 计算比例
    ratio = count_in_range / total_positive
    return ratio
...
class MultiSpike4(nn.Module):
    ...
    class quant4(torch.autograd.Function):
        ...
        @staticmethod
        def forward(ctx, input, training):
            ctx.save_for_backward(input)

            clamped = torch.clamp(input, min=0, max=1)

            # if training:
            #     rand = torch.rand_like(clamped)
            #     result = torch.zeros_like(clamped)

            #     mask_low = clamped <= 0.25
            #     mask_high = clamped >= 0.75
            #     mask_mid = (~mask_low) & (~mask_high)

            #     result[mask_low] = 0
            #     result[mask_high] = 1
            #     result[mask_mid] = (rand[mask_mid] > 0.5).float()
            # else:
            result = torch.zeros_like(clamped)
    ...
    def forward(self, x):
        return self.quant4.apply(x, self.training)
