"""
最开始的版本，训练和测试的时候都是随机的，导致测试的时候每次结果都不一样
"""
import torch
...
class vit_snn(nn.Module):
    def __init__(self,
        ...
        self.final_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='torch')
        self.final_conv = nn.Conv2d(embed_dims, embed_dims, kernel_size=3, stride=1, padding=1, bias=False)
        self.final_bn = nn.BatchNorm2d(embed_dims)
    ...
    def forward_features(self, x):
        ...
        T, B, C, H, W = x.shape
        x = self.final_lif(x)
        x = self.final_conv(x.flatten(0, 1))
        x = self.final_bn(x).reshape(T, B, C, H, W)
