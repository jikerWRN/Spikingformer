...
class MultiSpike4(nn.Module):
    ...
    def _get_selected_upper_bound(self):
        ...
        return self.checkpoint_upper_bound_mean.reshape(-1)[percentile_idx].clamp_min(1e-6)

    def get_current_upper_bound(self, x):
        upper_bound = self._get_selected_upper_bound()
    ...
    def get_shared_upper_bound(self, x, bn=None):
        ...
        return self.get_current_upper_bound(x)
...
class mem_update(nn.Module):
    ...
    def get_current_upper_bound(self, x):
        return self.qtrick.get_current_upper_bound(x)
...
class SpikingSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q_upper_bound = self.q_lif.get_current_upper_bound(q_conv_out)
        ...
        k_upper_bound = self.k_lif.get_current_upper_bound(k_conv_out)
        ...
        v_upper_bound = self.v_lif.get_current_upper_bound(v_conv_out)
        v = v_conv_out.transpose(-1, -2).reshape(T, B, N, self.num_heads, C // self.num_heads)
        v = v.permute(0, 1, 3, 2, 4)

        attn = q @ k.transpose(-2, -1)
        scale = (q_upper_bound * k_upper_bound * v_upper_bound).clamp_min(1e-6)
        x = ((attn @ v) * 0.125) / scale
