...
class AdaptiveMultiStepLIFNode(MultiStepLIFNode):
    class QuantUpperBoundSpike(torch.autograd.Function):
        ...
        def backward(ctx, grad_output):
            ...
            grad_input[input > 1.5 * checkpoint_upper_bound] = 0
...
class SpikingTokenizer(nn.Module):
    def __init__(
        ...
        self.block3_conv = nn.Conv2d(embed_dims // 2, embed_dims // 1, kernel_size=3, stride=2, padding=1, bias=False)
        ...
        self.block4_conv = nn.Conv2d(embed_dims, embed_dims, kernel_size=3, stride=2, padding=1, bias=False)
